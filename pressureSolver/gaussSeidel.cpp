...
{
    double factor = 0.5
    * (std::pow(discretization_->meshWidt[0], 2) * std::pow(discretization_->meshWidt[1], 2)) 
    / (std::pow(discretization_->meshWidt[0], 2) + std::pow(discretization_->meshWidt[1], 2));
        ...
        for (int i = 1; i < discretization_->.p.sizeX() - 1; ++i)
        {
            discretization_->p(i,j) = factor 
                * ((discretization_->p(i-1,j) + discretization_->p(i+1,j)) 
                    / std::pow(discretization_->meshWidt[0], 2)
                    + (discretization_->p(i,j-1) + discretization_->p(i,j+1)) 
                    / std::pow(discretization_->meshWidt[1], 2) 
            ...
            residual += std::pow(
                rhs(i,j) 
                    - (p(i-1,j) - 2 * p(i,j) + p(i+1,j)) 
                        / std::pow(meshWidt[0],2) 
                    - (p(i,j-1) - 2 * p(i,j) + p(i,j+1)) 
                        / std::pow(meshWidt[1],2)
                ,2);
        }
    }
